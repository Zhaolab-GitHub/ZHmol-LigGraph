...
def _pdb_file_to_pose(pdb, pdb_file, Atoms, Bonds, bond_th, nucleic_gt, tot, tile_size,
    ...
    for st in f:
        ...
        if (ss[0] == 'ENDMDL'):
            num_nodes = gen_3D_2_pose_atomwise(nucleic_gt, ligand, Atoms, Bonds, edge_gt, bond_th, output_dir+"/"+str(file_counter))

            if num_nodes != len(gt_pose) + len(nucleic_gt):
                print(f"pose has {num_nodes} nodes while gt has {len(gt_pose)} nodes. nucleic_gt {pdb_file[-8:-4]},  {len(ligand_gt)}")
                print(f"{len(nucleic_gt)} {len(ligand)} {len(gt_pose)}")
                print([line[-1] for line in nucleic_gt])
...
def read_pdbbind_to_disk_rmsd_energy_split(input_list, groundtruth_dir, groundtruth_suffix, pdbbind_dir, output_dir, resolution,
                                           tile_size, bond_th, pocket_th, pdb_id_st, pdb_id_ed, seed = None):
    rec_list = []
    label_list = []
    ...
    for i in range(pdb_id_st, pdb_id_ed):
        ...
        pdb = pdb_list[i]
        gt_pose, nucleic_gt, ligand_gt, edge_gt = file_to_gt_pose(groundtruth_dir, groundtruth_suffix, pdb, Atoms, Bonds, pocket_th)
        pdb_file = os.path.join(pdbbind_dir, pdb+'.pdb')
        tot, file_counter, global_tot = _pdb_file_to_pose(pdb, pdb_file, Atoms, Bonds, bond_th, nucleic_gt, tot, tile_size,
                                                          global_tot, output_dir, gt_pose, ligand_gt, edge_gt, file_counter)
        pbar.update(1)
    pbar.close()
