...
def file_to_gt_pose(groundtruth_dir, groundtruth_suffix, pdb, Atoms, Bonds, pocket_th):
    ...
    for st in f:
        ...
        if (tt == 2):
            ...
            if len(ss) > 1:
                ...
                print(f"Invalid entry in ss: {ss}")
            if len(ss) > 1:
    ...
    for st in f:
        ss = st.split()
        if (ss[0] == 'ATOM'):
            name, x, y, z, atom, idx = line_to_coor(st, 'nucleic_atom')
            if (atom != 'H'):
                nucleic_gt.append((name, x, y, z, atom, idx))
                if name == 'CA' and distance.euclidean([x, y, z], [cx, cy, cz]) < pocket_th:
                    pocket_idx.append(idx)
    f.close()
    nucleic_gt = [line for line in nucleic_gt if line[5] in pocket_idx]

    gt_pose = gen_3D_2_gt_pose(nucleic_gt, ligand_gt, Atoms, file_dir=None, use_nucleic=False)
    return gt_pose, nucleic_gt, ligand_gt, edge_gt


def gen_3D_2_gt_pose(nucleic, ligand, Atoms, file_dir, use_nucleic = True):
...
def gen_3D_2_pose(nucleic, ligand, Atoms, Bonds, bond_th, file_dir):
...
def gen_3D_2_pose_atomwise(nucleic, ligand, Atoms, Bonds, edge_gt, bond_th, file_dir):
    """ Convert a pose with pdb format to graph format"""
